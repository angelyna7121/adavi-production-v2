...
const { calculateReconciliation, confirmedDetailRows } = await import("../app/lib/reportData.ts");
...
line("REAL ESTATE");line("1177 Yonge St. Toronto","300,000","300,000");
line("INVESTMENTS");line("Suffolk LP investment","3,969,755","3,969,755");
...
test("produces and reconciles the scanned net worth statement totals",()=>{const currentAssets=leaves.filter((r)=>r.kind==="Asset").reduce((s,r)=>s+r.current,0);const currentLiabilities=leaves.filter((r)=>r.kind==="Liability").reduce((s,r)=>s+r.current,0);const previousAssets=leaves.filter((r)=>r.kind==="Asset").reduce((s,r)=>s+(r.previous??0),0);const previousLiabilities=leaves.filter((r)=>r.kind==="Liability").reduce((s,r)=>s+(r.previous??0),0);assert.equal(currentAssets,23743567);assert.equal(currentLiabilities,7424244);assert.equal(currentAssets-currentLiabilities,16319323);assert.equal(previousAssets-previousLiabilities,16319426);assert.deepEqual(calculateReconciliation(leaves),{calculated:16319323,source:16319323,difference:0,matches:true});});
