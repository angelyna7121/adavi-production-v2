...
test("keeps two assignments of the same source independent",()=>{const source=[{...rows[0],id:"ocr-row",sourceCurrentNetWorth:1000,current:1000,previous:900}];const jay=assignStatement(assignInvestor(source,"jay","Jay"),{statementId:"jay-suffolk",investorId:"jay",filename:"Suffolk.pdf",ownershipPercentage:34,parseStatus:"ready"});const cindy=assignStatement(assignInvestor(source,"cindy","Cindy"),{statementId:"cindy-suffolk",investorId:"cindy",filename:"Suffolk.pdf",ownershipPercentage:100,parseStatus:"ready"});assert.notEqual(jay[0].id,cindy[0].id);assert.deepEqual([jay[0].current,cindy[0].current],[340,1000]);const edited=[{...jay[0],description:"Jay edit",include:false},...cindy];assert.equal(edited.find((row)=>row.statementId==="cindy-suffolk").description,source[0].description);assert.equal(edited.find((row)=>row.statementId==="cindy-suffolk").include,true);assert.equal(calculateAssignmentReconciliations(edited).length,2);});
