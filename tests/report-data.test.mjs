...
test("exports exact headers and leaf accounts only",()=>{const csv=createReportCsv([...rows,{...rows[0],id:"t",description:"Total Assets",accountName:"Total Assets",current:2935282.7}]);assert.equal(csv.codePointAt(0),0xfeff);assert.ok(csv.endsWith("\r\n"));const lines=csv.slice(1).trimEnd().split("\r\n");const parse=(line)=>[...line.matchAll(/(?:^|,)(?:"((?:""|[^"])*)"|([^,]*))/g)].map((m)=>(m[1]??m[2]).replaceAll('""','"'));assert.deepEqual(parse(lines[0]),[...CSV_HEADERS]);assert.equal(lines.length,8);assert.ok(!csv.includes("Total Assets"));assert.deepEqual(parse(lines[1]).slice(0,6),["Ari Family","Asset","Cash and Bank Accounts","CIBC Bank","Bank Account","Bank Account"]);});
