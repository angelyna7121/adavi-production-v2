...
test("packs default print pages without empty, heading-only, or subtotal-only pages",()=>{const many=Array.from({length:55},(_,index)=>({...rows[3],id:`dense-${index}`,description:`Dense account ${index+1}`,accountName:`Dense account ${index+1}`,current:1000+index,previous:900+index}));const pages=paginateReportSchedule(groupReportRows(many));assert.equal(pages.length,3);assert.ok(pages.every((page)=>page.some((row)=>row.level==="account")));assert.ok(pages.every((page)=>page.length<=28));for(const page of pages){const firstAccount=page.findIndex((row)=>row.level==="account");assert.ok(firstAccount>=0);assert.ok(page.slice(0,firstAccount).every((row)=>row.level!=="total"));}const totalPage=pages.find((page)=>page.some((row)=>row.level==="total"));assert.ok(totalPage.some((row)=>row.level==="account"));});
