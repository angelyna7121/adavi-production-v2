...
test("uses the same fixed current and previous columns for headings, rows, and totals",()=>{assert.match(css,/\.accountTable td:nth-child\(2\),\.accountTable td:nth-child\(3\)\s*\{\s*width:160px;/);assert.match(css,/\.categorySubtotal\s*\{[^}]*grid-template-columns:minmax\(0,1fr\) 160px 160px/);assert.match(css,/\.disclosureToggle\s*\{[^}]*grid-template-columns:auto minmax\(0,1fr\) minmax\(100px,160px\) minmax\(100px,160px\)/);});
