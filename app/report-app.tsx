...
function StatementTableHead({currentLabel,previousLabel}:{currentLabel:string;previousLabel:string}){return <thead><tr className="periodGroupRow"><th scope="col">Investment / Account</th><th scope="col" className="ownershipCell">Ownership</th><th scope="col">Current · {currentLabel}<small>100% Value · Investor Share</small></th><th scope="col">Previous · {previousLabel}<small>100% Value · Investor Share</small></th></tr></thead>;}
function PeriodValues({full,share}:{full?:number;share?:number}){return <td className="periodCell"><span><small>100%</small><strong>{full===undefined?"":money(full)}</strong></span><span><small>Share</small><strong>{share===undefined?"":money(share)}</strong></span></td>;}
function StatementValues({row}:{row:{ownership?:number;currentFull?:number;currentShare?:number;previousFull?:number;previousShare?:number}}){return <><td className="ownershipCell">{row.ownership===undefined?"":ownership(row.ownership)}</td><PeriodValues full={row.currentFull} share={row.currentShare}/><PeriodValues full={row.previousFull} share={row.previousShare}/></>;}
