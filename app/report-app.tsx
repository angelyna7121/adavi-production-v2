...
import { assignInvestor, calculateAssetMix, calculatePeriodTotals, calculateReconciliation, confirmedDetailRows, createReportCsv, groupReportRows, hasUnnamedIncludedRows, paginateReportSchedule, type ReportGroup } from "./lib/reportData";
      ...
      <div className="shell">
        ...
        {step === "report" && (
          <section className="reportArea">
            ...
            <PrintReport groupName={groupName} statementDate={statementDate} currentLabel={currentPeriodLabel} previousLabel={previousPeriodLabel} assets={assets} previousAssets={previousAssets} liabilities={liabilities} previousLiabilities={previousLiabilities} netWorth={netWorth} previousNetWorth={previousNetWorth} netWorthChange={netWorthChange} groups={reportGroups} assetMix={assetMix} paid={hasVerifiedPaidEntitlement} />
          </section>
        )}
      </div>
...
function PrintLogo(){return <div className="printLogo"><span className="brandMark">ϟ</span><strong>adavi.ai</strong></div>;}
function PrintFooter({page,total,paid}:{page:number;total:number;paid:boolean}){return <footer className="printPageFooter"><span>{paid?"": "Created with adavi.ai"}</span><span>Page {page} of {total}</span></footer>;}
function PrintReport({groupName,statementDate,currentLabel,previousLabel,assets,previousAssets,liabilities,previousLiabilities,netWorth,previousNetWorth,netWorthChange,groups,assetMix,paid}:{groupName:string;statementDate:string;currentLabel:string;previousLabel:string;assets:number;previousAssets:number;liabilities:number;previousLiabilities:number;netWorth:number;previousNetWorth:number;netWorthChange:number;groups:ReportGroup[];assetMix:Array<{category:string;total:number;percentage:number}>;paid:boolean}){
  const schedulePages=useMemo(()=>paginateReportSchedule(groups),[groups]);const totalPages=schedulePages.length+2;const branding=paid?"paidPrint":"freePrint";
  return <div className={`printReport ${branding}`}>
    <section className="printPage executivePage">{!paid&&<div className="pageWatermark">adavi.ai</div>}<PrintLogo/><div className="printHero"><p>Statement of Net Worth</p><h1>Consolidated Net Worth Report</h1><h2>{groupName}</h2><div><span>Statement date</span><strong>{statementDate}</strong><span>Currency</span><strong>CAD</strong></div></div><div className="printSummaryCards"><Summary label="Total Assets" current={money(assets)} previous={money(previousAssets)} currentLabel={currentLabel} previousLabel={previousLabel} tone="assets"/><Summary label="Total Liabilities" current={money(liabilities)} previous={money(previousLiabilities)} currentLabel={currentLabel} previousLabel={previousLabel} tone="liabilities"/><Summary label="Net Worth" current={money(netWorth)} previous={money(previousNetWorth)} currentLabel={currentLabel} previousLabel={previousLabel} change={money(netWorthChange)} tone="networth"/></div>{!paid&&<div className="printDisclaimer">Educational estimates only · Not tax, legal, accounting, or investment advice.</div>}<PrintFooter page={1} total={totalPages} paid={paid}/></section>
    {schedulePages.map((rows,index)=><section className="printPage schedulePage" key={`schedule-page-${index}`}>{!paid&&<div className="pageWatermark">adavi.ai</div>}<header className="schedulePageHead"><PrintLogo/><div><h1>Detailed Consolidated Report</h1><p>{groupName} · {statementDate} · CAD</p></div></header><table className="printSchedule"><colgroup><col/><col className="printAmountColumn"/><col className="printAmountColumn"/></colgroup><thead><tr><th>Account / Description</th><th>Current</th><th>Previous</th></tr></thead><tbody>{rows.map((row)=><tr className={`printRow-${row.level}`} key={row.key}><th scope="row">{row.label}</th><td>{row.current===undefined?"":money(row.current)}</td><td>{row.previous===undefined?"":money(row.previous)}</td></tr>)}</tbody></table><PrintFooter page={index+2} total={totalPages} paid={paid}/></section>)}
    <section className="printPage assetMixPage">{!paid&&<div className="pageWatermark">adavi.ai</div>}<header className="schedulePageHead"><PrintLogo/><div><h1>Asset Mix</h1><p>{groupName} · {statementDate} · CAD</p></div></header><div className="printAssetMix">{assetMix.map((entry)=><div className="printMix" key={entry.category}><div><strong>{entry.category}</strong><span>{money(entry.total)}</span><em>{entry.percentage.toFixed(1)}%</em></div><div className="printMixTrack"><i style={{width:`${entry.percentage}%`}}/></div></div>)}</div>{!paid&&<div className="printDisclaimer">Educational estimates only · Not tax, legal, accounting, or investment advice.</div>}<PrintFooter page={totalPages} total={totalPages} paid={paid}/></section>
  </div>;
}
