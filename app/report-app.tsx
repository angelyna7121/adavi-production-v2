...
import { applyOwnershipToRow, assignInvestor, assignStatement, calculateAssetMix, calculatePeriodTotals, calculateReconciliation, confirmedDetailRows, createReportCsv, duplicateFinancialRow, effectiveOwnershipPercentage, groupReportRows, hasUnnamedIncludedRows, removeInvestorRows, removeStatementRows, setRowOwnershipOverride, setStatementOwnership, validateOwnershipPercentage, type ReportGroup, type StatementRecord } from "./lib/reportData";
...
function PrintReport({groupName,statementDate,currency,currentLabel,previousLabel,assets,previousAssets,liabilities,previousLiabilities,netWorth,previousNetWorth,netWorthChange,assetMix,paid,hasPartialOwnership}:{groupName:string;statementDate:string;currency:string;currentLabel:string;previousLabel:string;assets:number;previousAssets:number;liabilities:number;previousLiabilities:number;netWorth:number;previousNetWorth:number;netWorthChange:number;assetMix:Array<{category:string;total:number;percentage:number}>;paid:boolean;hasPartialOwnership:boolean}){
  const totalPages=2;const branding=paid?"paidPrint":"freePrint";
