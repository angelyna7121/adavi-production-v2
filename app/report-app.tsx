        ...
        {step === "report" && (
          <section className="reportArea">
            ...
            <article className={`paper ${hasVerifiedPaidEntitlement ? "paidReport" : "freeReport"}`}>
              ...
              <div className="detailedStatement">{reportGroups.map((investor) => <section className="investorGroup" key={investor.investor}><h4>{investor.investor}</h4>{investor.sections.map((section) => <section className="statementSection" key={section.kind}><h5>{section.kind === "Asset" ? "Assets" : "Liabilities"}</h5>{section.categories.map((category) => <Disclosure className="categoryGroup" label={category.category} current={money(category.total)} previous={money(category.previousTotal)} showColumnLabels key={category.category}>{category.holders.map((holder) => <Disclosure className="holderGroup" label={holder.holder} current={money(holder.total)} previous={money(holder.previousTotal)} key={holder.holder}><table className="reportTable accountTable"><tbody>{holder.rows.map((row) => <tr key={row.id}><td><strong>{row.description}</strong></td><td>{money(row.current)}</td><td>{money(row.previous)}</td></tr>)}</tbody></table></Disclosure>)}<div className="categorySubtotal"><span>Total {category.category}</span><strong>{money(category.total)}</strong><strong>{money(category.previousTotal)}</strong></div></Disclosure>)}</section>)}</section>)}</div>
...
function Disclosure({ className, label, current, previous, showColumnLabels = false, children }: { className: string; label: string; current: string; previous: string; showColumnLabels?: boolean; children: React.ReactNode }) {
  const [expanded, setExpanded] = useState(true);
  return <section className={className}><button type="button" className="disclosureToggle" aria-expanded={expanded} onClick={() => setExpanded((value) => !value)}><span className="disclosureLabel">{label}</span>{showColumnLabels ? <><span className="periodHeading">Current</span><span className="periodHeading">Previous</span></> : <><strong>{current}</strong><strong>{previous}</strong></>}</button><div className={`disclosureContent ${expanded ? "expanded" : "collapsed"}`}>{children}</div></section>;
}
