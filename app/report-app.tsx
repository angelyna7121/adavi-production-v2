...
import { applyOwnershipToRow, assignInvestor, assignStatement, calculateAssetMix, calculatePeriodTotals, calculateReconciliation, confirmedDetailRows, createReportCsv, filterPrintableSchedulePages, groupReportRows, hasUnnamedIncludedRows, paginateReportSchedule, removeInvestorRows, removeStatementRows, setStatementOwnership, validateOwnershipPercentage, type ReportGroup, type StatementRecord } from "./lib/reportData";
...
export default function ReportApp({ hasVerifiedPaidEntitlement }: { hasVerifiedPaidEntitlement: boolean }) {
  ...
  const hasPartialOwnership = statements.some((statement) => statement.ownershipPercentage < 100);
  ...
  function addRow() {
    ...
    setRows((current) => [...current, { id: crypto.randomUUID(), include: true, investor: investor?.name.trim() ?? "", investorId: investor?.investorId, ownershipPercentage:100, rawCurrent:"", rawPrevious:null, category: "Other Assets", holder: "", accountName: "", institution: "", description: "", current: "", previous: null, kind: "Asset", source: "Manual entry" }]);
  }
        ...
        {step === "review" && (
          <section className="workspace reviewSpace">
            ...
            <div className="tableWrap" tabIndex={0} aria-label="Review extracted financial rows">
              <table className="reviewTable"><thead><tr><th>Include</th><th>Investor</th><th>Category</th><th>Holder / Institution</th><th>Account / Description</th><th>Statement / Ownership</th><th>Raw Current</th><th>Included Current</th><th>Raw Previous</th><th>Included Previous</th><th>Asset / Liability</th><th></th></tr></thead>
                <tbody>{rows.map((row) => <tr key={row.id} className={`${row.needsReview ? "needsReview" : ""} ${(row.ownershipPercentage??100)<100?"ownershipAdjusted":""}`}>
                  ...
                  <td className="sourceCell"><strong>{row.source}</strong><span className={(row.ownershipPercentage??100)<100?"ownershipBadge adjusted":"ownershipBadge"}>{row.ownershipPercentage??100}% ownership{(row.ownershipPercentage??100)<100?" applied":""}</span></td>
...
function PrintReport({groupName,statementDate,currency,currentLabel,previousLabel,assets,previousAssets,liabilities,previousLiabilities,netWorth,previousNetWorth,netWorthChange,groups,assetMix,paid,hasPartialOwnership}:{groupName:string;statementDate:string;currency:string;currentLabel:string;previousLabel:string;assets:number;previousAssets:number;liabilities:number;previousLiabilities:number;netWorth:number;previousNetWorth:number;netWorthChange:number;groups:ReportGroup[];assetMix:Array<{category:string;total:number;percentage:number}>;paid:boolean;hasPartialOwnership:boolean}){
