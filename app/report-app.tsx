...
function PrintFooter({page,total,paid}:{page?:number;total:number;paid:boolean}){return <footer className="printPageFooter"><span>{paid?"":"Created with adavi.ai"}</span><span>{paid?"":"Educational estimates only"}</span><span>{page?`Page ${page} of ${total}`:""}</span></footer>;}
