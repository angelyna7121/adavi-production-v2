...
export default function ReportApp({ hasVerifiedPaidEntitlement }: { hasVerifiedPaidEntitlement: boolean }) {
  ...
  const printCurrentTotals = useMemo(() => calculatePeriodTotals(printRows, "current"), [printRows]);
  const printPriorTotals = useMemo(() => calculatePeriodTotals(printRows, "previous"), [printRows]);
  const printReportGroups = useMemo(() => groupReportRows(printRows), [printRows]);
      ...
      <div className="shell">
        ...
        {step === "report" && (
          <section className="reportArea">
            ...
            <PrintReport groupName={groupName} statementDate={statementDate} currency={currency} currentLabel={currentPeriodLabel} previousLabel={previousPeriodLabel} assets={printCurrentTotals.assets} previousAssets={printPriorTotals.assets} liabilities={printCurrentTotals.liabilities} previousLiabilities={printPriorTotals.liabilities} netWorth={printCurrentTotals.netWorth} previousNetWorth={printPriorTotals.netWorth} netWorthChange={printCurrentTotals.netWorth-printPriorTotals.netWorth} reportGroups={printReportGroups} assetMix={printAssetMix} paid={hasVerifiedPaidEntitlement} hasPartialOwnership={printRows.some((row)=>effectiveOwnershipPercentage(row)<100)} />
          </section>
        )}
      </div>
      {rowEditor&&<RowEditorDialog editor={rowEditor} investors={investorList} statements={statements} error={rowEditorError} onPatch={patchEditor} onRaw={patchEditorRaw} onOwnership={patchEditorOwnership} onCancel={()=>{setSelectedRowId(rowEditor.sourceRowId??selectedRowId);setRowEditor(null);setRowEditorError("");}} onSave={saveRowEditor}/>}
...
function RowEditorDialog({editor,investors,statements,error,onPatch,onRaw,onOwnership,onCancel,onSave}:{editor:RowEditor;investors:Investor[];statements:StatementRecord[];error:string;onPatch:(patch:Partial<Row>)=>void;onRaw:(period:"current"|"previous",value:number|""|null)=>void;onOwnership:(value:number|null)=>void;onCancel:()=>void;onSave:()=>void}){
