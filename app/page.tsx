        ...
        {step === "report" && (
          <section className="reportArea">
            ...
            <article className="paper">
              ...
              <table className="reportTable"><thead><tr><th>Category / Description</th><th>Investor</th><th>Current Period</th><th>Previous Period</th><th>Change</th></tr></thead><tbody>{included.map((row) => <tr key={row.id}><td><strong>{row.description}</strong><small>{row.category} · {row.institution || "No institution"} · {row.kind}</small></td><td>{row.investor}</td><td>{money(row.current)}</td><td>{money(row.previous)}</td><td>{row.previous === null ? "N/A" : money(Number(row.current) - row.previous)}</td></tr>)}</tbody><tfoot><tr><td colSpan={2}>Consolidated Net Worth</td><td>{money(netWorth)}</td><td>{hasPrevious ? money(previousNetWorth) : "N/A"}</td><td>{hasPrevious ? money(netWorth - previousNetWorth) : "N/A"}</td></tr></tfoot></table>
