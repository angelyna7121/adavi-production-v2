...
export function validateOwnershipPercentage(value:number){if(!Number.isFinite(value)||value<0||value>100)throw new Error("Ownership percentage must be between 0 and 100.");return value;}
...
export function assignStatement(rows:ParsedRow[],statement:StatementRecord){return rows.map((row)=>applyOwnershipToRow({...row,id:`${statement.statementId}:${row.id}`,statementId:statement.statementId,source:statement.filename},statement.ownershipPercentage));}
...
export function calculateAssignmentReconciliations(rows:ParsedRow[]){
  const assignments=[...new Set(rows.map((row)=>row.statementId).filter((value):value is string=>Boolean(value)))];
  return assignments.map((statementId)=>{const assignmentRows=rows.filter((row)=>row.statementId===statementId);const reconciliation=calculateReconciliation(assignmentRows);return {statementId,investorId:assignmentRows[0]?.investorId,investor:assignmentRows[0]?.investor??"",source:assignmentRows[0]?.source??"",calculated:reconciliation.calculated,sourceTotal:reconciliation.source,difference:reconciliation.difference,matches:reconciliation.matches};});
}
